...
export default function MintArticle() {
  ...
  const handleUploadAndMint = async () => {
    ...
    try {
      setShowUploadModal(true); // Show modal for image upload

      const uploadImage = await axios.post('https://api.pinata.cloud/pinning/pinFileToIPFS', formData, {
      ...
      <div className="px-4 py-8 min-h-screen bg-gray-50 flex flex-col items-center">
        <div className="w-full max-w-2xl bg-white shadow-lg border border-gray-200 rounded-lg p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-bold">Mint your Article as NFT</h1>
          ...
          {/* Title */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Article Headline (NFT Title)</label>
            <input
              type="text"
              value={titleText}
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-transparent backdrop-blur-md bg-opacity-20 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3 text-center border border-zinc-200">
            ...
            <p className="text-xs mb-4 text-zinc-500 font-semibold">Please do not refresh or close page until it's completed.</p>
        ...
        <div className="fixed inset-0 bg-transparent backdrop-blur-md bg-opacity-20 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3 text-center border border-zinc-200">
            <h2 className="text-lg font-semibold mb-4">
              {mintingSuccess ? 'Minting Successful!' : 'Please approve from your wallet'}
            </h2>
            <p className="mb-4">
              {mintingSuccess ? 'Your NFT has been successfully minted!' : 'Waiting for wallet approval to complete the minting.'}
            </p>
            <button
